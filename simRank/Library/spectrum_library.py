...
from Library.psm_library import *
import re
...
def load_frag_db(
    ...
    for file in os.listdir(foldername)[index_low:index_up]:
        ...
        if file.lower().endswith(".mgf") or file.lower().endswith(".mzxml"):
            ...
            SpecCol.load_from_file(
                peaktable=peaktable,
                drop_mslevels=drop_mslevels,
                inten_thresh=inten_thresh,
                engine=engine,
                remove_pre=remove_pre,
                min_mz_num=min_mz_num,
            )
            if num % 100 == 0:
                print(
                    "\rprocessor {}, {} files loaded".format(processor_num, num),
                    end="",
                    flush=True,
                )
            num += 1
...
class Database:
    ...
    def load_files(
        ...
        nm_ls = os.listdir(self.foldername)
        file_num = len(nm_ls)
        n_cpu = n_workers
    ...
    def decompose_with_ions_in_mgf(self, root):
        ...
        for coll in self.SpectrumCollection_list:
            ...
            num += 1
            if num % 100 == 0:
                print("Num:{}, Now it is ".format(num) + coll.global_paras["MID"])
        ...
        for coll in self.SpectrumCollection_list:
            ...
            with open(file_path, "w", encoding="utf-8") as f:
                ...
                for spectrum in coll.spectrum_list:
                    if spectrum != None:
                        f.write(spectrum.get_mgf_string(with_energy=with_energy) + "\n")
            if num % 100 == 0:
                print("Num:{}, Now it is ".format(num) + coll.global_paras["MID"])
            num += 1
    ...
    def combine_database(self, db):
        ...
        if len(db.SpectrumCollection_list) > 0:
            combined_db = Database(
                foldername="", DBname=str(self.name) + "+" + str(db.name)
            )
    ...
    def get_frag_feature(self, minmz=30, maxmz=5000):
        ...
        for i, spec_c in enumerate(self.SpectrumCollection_list):
            if (i + 1) % 100 == 0:
                print(
                    "\rGetting fragment features, file num:{}".format(i + 1),
                    end="",
                    flush=True,
                )
            for spec in spec_c.spectrum_list:
        ...
        self.frag_feature = list(map(lambda a: round(float(a), 2), self.frag_feature))
        self.frag_feature = list(set(self.frag_feature))
        frag_feature = np.asarray(self.frag_feature)
        frag_feature = frag_feature[
            np.logical_and(frag_feature >= minmz, frag_feature <= maxmz)
        ]
    ...
    def get_loss_feature(self, min_loss_mz=14, max_loss_mz=5000):
        ...
        for spec_c in self.SpectrumCollection_list:
            ...
            if i % 100 == 0:
                print(
                    "\rGetting loss features, file num:{}".format(i), end="", flush=True
                )
            for spec in spec_c.spectrum_list:
                spec.cal_neutral_loss(min_loss_mz)
                peaks = spec.neutral_loss[:, 0]
                self.loss_feature.extend(list(peaks))
        self.loss_feature = list(map(lambda a: round(float(a), 2), self.loss_feature))
        ...
        loss_feature = loss_feature[
            np.logical_and(loss_feature >= min_loss_mz, loss_feature <= max_loss_mz)
        ]
        ...
        for index in range(start, end):
            ...
            for spec in spec_c.spectrum_list:
                ...
                for feature, intensity in pks_frag:
                    c_frag[np.where(self.frag_feature == feature)[0]] = round(
                        intensity, 2
                    )
                for feature, intensity in pks_loss:
                    c_loss[np.where(self.loss_feature == feature)[0]] = round(
                        intensity, 2
                    )
                ...
                if num % 100 == 0:
                    print(
                        "\rprocessor {} is processing the collection {}".format(i, num),
                        end="",
                        flush=True,
                    )
    ...
    def get_LDA_matrix(
        self, minmz=30, maxmz=5000, min_loss_mz=14, max_loss_mz=5000, iontype="[M+H]+"
    ):
        """
        ...
        :return:
        """
        self.get_LDA_feature(
            minmz=minmz, maxmz=maxmz, min_loss_mz=min_loss_mz, max_loss_mz=max_loss_mz
        )
        # if self.loss_feature is None:
        #     self.get_loss_feature(min_loss_mz, max_loss_mz)
        # if self.frag_feature is None:
        ...
        if n_col < n_cpu:
            for i in range(n_col):
                Ms.append(
                    pool.apply_async(self.load_frag_loss_M, (i, i + 1, i, iontype))
                )
        else:
            for i in range(n_cpu):
                # if i < num:
                if i + 1 < n_cpu:
                    ...
                    Ms.append(
                        pool.apply_async(
                            self.load_frag_loss_M, (i * base, n_col, i, iontype)
                        )
                    )
        pool.close()
        pool.join()
        ...
        print("")
        loss_time = time.time()
        print("LDA Matrix Construction time consuming: {}s".format(loss_time - st_time))
        print("-----------------------------------------------------------------------")
        self.total_feature_M = np.concatenate((self.frag_M, self.loss_M), axis=0)
    ...
    def compare_Mass(self, mass_dic):
        ...
        if len(self.SpectrumCollection_list) == 0:
            print("load file first")
            return
        self.SpectrumCollection_list.sort(key=lambda a: a.MID)  # sort spectra by MID
        MID_new = set(list(mass_dic.keys()))
        MID_origin = set(self.MID_ls)
        MID_common = list(MID_new.intersection(MID_origin))
        MID_common.sort()
        ...
        for Spec_c in self.SpectrumCollection_list:
            if Spec_c.MID == last_MID:
                ...
                if (
                    abs(
                        float(Spec_c.global_paras["Mass"]) - mass_dic[MID_common[index]]
                    )
                    > 0.01
                ):
...
class SpectrumCollection:
    ...
    def load_from_mgf(self, inten_thresh=0, engine="UTF-8", min_mz_num=6):
        ...
        for line in lines:
            ...
            if mgf_file_line == "END IONS":
                ...
                if len(peaks) > 0:
                    ...
                    if inten_thresh is not None:
                        ...
                        real_thresh = (
                            inten_thresh / 100.0 * max(adding_spectrum.peaks[:, 1])
                        )
                        adding_spectrum.filter_by_intensity(inten_thresh=real_thresh)
            ...
            if mgf_file_line.split("=")[0] == "TITLE":
                try:
                    ...
                    energy = float(mgf_file_line.split(" ")[0].split("=")[1][:-1])
                    # flag is a char
                    ion_info = ion.split("M")[1].split("]")[
                        0
                    ]  # ion_info indicates addition
                    flag = ion.split("M")[0].split("[")[1]  # flag indicate how many M
                    if len(flag) < 1:
                        flag = "1"
                    ion_pair = (
                        flag,
            ...
            if mgf_file_line.split("=")[0] == "ION":
                try:
                    ...
                    print("ION line info is not given or cannot be identified")
                    continue
        ...
        self.spectrum_list = output_spectra
        self.MID = str(os.path.basename(self.filename).split(".")[0][0:].split("_")[0])
        self.global_paras[
            "MID"
        ] = (
            self.MID
        )  # in some mgf files, the MID has no suffix, make sure MID is based on filename suffix
    ...
    def load_from_mzXML(
                    ...
                    if len(peaks) >= min_mz_num:
                        ...
                        if inten_thresh is not None:
                            if type(output.peaks) is not np.ndarray:
                                output.peaks = np.asarray(output.peaks, dtype=np.float)
                            real_thresh = inten_thresh / 100.0 * max(output.peaks[:, 1])
        ...
        def load_mzxml_file(
            filename, drop_mslevels=[1], peaktable=False, ppm=20, rt=30, engine=engine
        ):
            output_ms2 = []
                ...
                for i, scan in enumerate(read_scans):
                    ...
                    if scan.get("@msLevel", -1) == "2":
                        if peaktable is not False:
                            ...
                            retention = round(float(scan["@retentionTime"][2:-1]), 2)
                            mzlist = np.array(peaktable["mz"].astype("float"))
                            mzlist = list(abs((mzlist - precursormass) / precursormass))
                            ...
                            for j in range(len(mzlist)):
                                if mzlist[j] < (ppm / 1e06) and rtlist[j] < rt:
                                    match = True
                                    break
                        if ((peaktable is not False) and (match)) or (
                            peaktable is False
                        ):
                            (
                                ms_level,
                                ...
                                scan, index, struct_iter_ok, canary, drop_mslevels
                            )
                            if ms_level == 2 and spectrum is not None:
                                output_ms2.append(spectrum)
                                index += 1
                    print(
                        "\r{}, {} scans loaded, progressing: {}%".format(
        ...
        self.spectrum_list = load_mzxml_file(
            self.filename,
            drop_mslevels=drop_mslevels,
            peaktable=peaktable,
            ppm=ppm,
        ...
        for spectrum in self.spectrum_list:
            ...
            num += 1
            print(
                "\r{}, {} mslist constructed, progressing: {}%".format(
    ...
    def load_ms1_mzxml_file(
        ...
        for spec in output_ms1:
            if up_scan is not None and (spec.scan > up_scan or spec.scan < low_scan):
                continue
            if up_rt is not None and (
                spec.retention_time > up_rt or spec.retention_time < low_rt
            ):
                continue
        ...
        if len(r) > 0:
            ...
            if if_norm:
                merged_spec[:, 1] = merged_spec[:, 1] / (max(merged_spec[:, 1]))
            return merged_spec
        else:
            return None
    ...
    def merge_mgf_spectra(self, ms2delta=0.01):
        ...
        i = 1
        while i < num_spectra:
            all_spec = np.concatenate((all_spec, self.spectrum_list[i].peaks), axis=0)
        ...
        for i in range(1, all_spec.shape[0]):
            if (all_spec[i, 0] - peak) < ms2delta:
                ...
                peak = all_spec[i, 0]
                inten = all_spec[i, 1]
        merged_spec = np.concatenate((merged_spec, np.asarray([[peak, inten]])), axis=0)
    ...
    def merge_mzxml_spectra(self, ppm=20, rt=30, ms2delta=0.01, by_col_e=True):
        ...
        """
        print("-----------------------------------------------------------------------")
        print("Start merging spectra" + " by Collision Energy" if by_col_e else "")
        ...
        if not by_col_e:
            ...
            while i < (num_spectra - 1):
                ...
                if (abs((mz1 - mz2) / mz2 * 1e6) < ppm) and (abs(rt1 - rt2) < rt):
                    ...
                    self.spectrum_list[i].filename = (
                        self.spectrum_list[i].filename
                        + "+"
                        + self.spectrum_list[i + 1].filename
                    )
                    ...
                    self.spectrum_list[i].precursor_intensity = round(
                        (
                            self.spectrum_list[i].precursor_intensity
                            + self.spectrum_list[i + 1].precursor_intensity
                        )
                        / 2,
                        1,
                    )
                    self.spectrum_list[i].totIonCurrent = (
                        self.spectrum_list[i].totIonCurrent
                        + self.spectrum_list[i + 1].totIonCurrent
                    )
                    ...
                    process = process + 1
                print(
                    "\r {} spectrums merged, Merging progressing: {}%".format(
                        process, round(process / total_process * 100, 2)
                    ),
                    end="",
                    flush=True,
                )
            ...
            col_e_ls.sort()
            spec_l_col = [
                [] for _ in range(len(col_e_ls))
            ]  # empty list of lists for spectrum lists corresponding to different collision energy
            ...
            for type, spec_l in enumerate(spec_l_col):
                ...
                if num_spectra >= 2:
                    ...
                    while i < (num_spectra - 1):
                        ...
                        if (abs((mz1 - mz2) / mz2 * 1e6) < ppm) and (
                            abs(rt1 - rt2) < rt
                        ):
                            # merge rows in ms1list
                            mz_new = round((mz1 + mz2) / 2, 4)
                            ...
                            process += 1
                        print(
                            "\rCollision Energy {}, {} spectrums merged, Merging progressing: {}%".format(
                                col_e_ls[type],
                                process + 1,
                                round((process + 1) / (total_process) * 100, 2),
                            ),
                            end="",
                            flush=True,
                        )
                ...
                try:
                    ...
                    for i in range(0, len(adducts), 2):
                        ...
                        if flag == "+":
                            ...
                            spec.mz = float(-1)
                            return
                    spec.mz = round(mz / abs(spec.charge), 4)  # divide by charge state
                except:
                    spec.mz = float(-1)
...
class Spectrum:
    ...
    def filter_by_intensity(self, inten_thresh=2):
        ...
        self.peaks = self.peaks[np.argwhere(self.peaks[:, 1] > inten_thresh)[:, 0]]
    ...
    def generated_spectrum_vector(
        self, peptide=None, attenuation_ratio=0.0, tolerance=0.5, bin_size=1
    ):
        ...
        if peptide != None:
            ...
            theoretical_peaks = create_theoretical_peak_map(
                self.peptide,
                ["b", "y", "b-H2O", "b-NH3", "y-H2O", "y-NH3", "a"],
                charge_set=charge_set,
            )
            annotated_peaks, unannotated_peaks = extract_annotated_peaks(
    ...
    def get_number_of_signal_peaks(self, SNR_Threshold=5):
        return numerical_utilities.calculate_signal_peaks_in_peaklist(
            self.peaks, SNR_Threshold
        )
    ...
    def cal_neutral_loss(self, min_loss_mz=14, max_loss_mz=5000):
        self.neutral_loss = np.stack((self.mz - self.peaks[:, 0], self.peaks[:, 1])).T
        self.neutral_loss = self.neutral_loss[
            np.where(self.neutral_loss[:, 0] >= min_loss_mz)[0]
        ]
        self.neutral_loss = self.neutral_loss[
            np.where(self.neutral_loss[:, 0] <= max_loss_mz)[0]
        ]
        return self.neutral_loss

    def remove_negative_neutral_loss(self):
        self.neutral_loss = self.neutral_loss[np.where(self.neutral_loss[:, 0] < 0)[0]]
